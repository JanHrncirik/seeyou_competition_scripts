Program Racing_task_Alternative_Scoring;

// Version 1.00 Date November 11, 2022 by Jan Hrncirik
//   .Script Racing task withou handicap – Alternative scoring
...
var
//Dt, Td, n1, n2, N, D0, V0, T0, Dm, Hmin, Pm, Pdm, Pvm, Pn, F, Fcr, Day: Double; CORRECTED; DELETED Dt, Td, Hmin, Pdm, Pvm, Pn; ADDED Sp0, Spm
          n1, n3, N, D0, V0, T0, Dm, Pm, F, Fcr, Sp0, Spm, Day: Double;
...
begin
  ...
  //Hmin := 100000;  DELETED Lowest Handicap of all competitors in the class
  ...
  //begin DELETED 
    ...
    //begin DELETED 
    //  If Pilots[i].Hcap < Hmin Then Hmin := Pilots[i].Hcap; // Lowest Handicap of all competitors in the class DELETED 
   //end; DELETED 
 //end; DELETED 
  //If Hmin=0 Then DELETED 
  //begin DELETED 
  // Info1 := 'Error: Lowest handicap is zero!'; DELETED 
  //	Exit; DELETED 
  //end; DELETED 

// BULK REPLACEMENT Hmin/Pilots[i].Hcap for 1
  for i:=0 to GetArrayLength(Pilots)-1 do
  begin
    If not Pilots[i].isHC Then
    begin
      If Pilots[i].dis*1 >= Dm Then n1 := n1+1;  // Competitors who have achieved at least Dm
 ...
 //   If not Pilots[i].isHC Then; DELETED
 //   begin; DELETED
 //     If Pilots[i].speed*1 > (2.0/3.0*V0) Then; DELETED
 //     begin; DELETED
 //       n2 := n2+1; DELETED
 //    end; DELETED
 //   end; DELETED
...
//Pdm := Pm-Pvm;                 // maximum available Distance Points for the Day; DELETED
  ...
  begin
    ...
    If Pilots[i].finish > 0 Then
    begin
    //Pv := Pvm * (Pilots[i].speed*1 - 2.0/3.0*V0)/(1.0/3.0*V0); CORRECTED to formula below
      Pv := 1000 * ((Pilots[i].speed*1)/V0);
    //If Pilots[i].speed*1 < (2.0/3.0*V0) Then Pv := 0; DELETED
    //Pd := Pdm; CORRECTED to formula below
      Pd := 750*((Pilots[i].dis*1)/D0);
    end
    Else
    //For any non-finisher
    begin
      Pv := 0;
    //Pd := Pdm * (Pilots[i].dis*1/D0); CORRECTED to formula below
      Pd := 750 * (Pilots[i].dis*1/D0);
    end;
